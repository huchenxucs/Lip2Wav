...
class TestFeeder:
	...
	def __init__(self, coordinator, hparams):
		super(TestFeeder, self).__init__()
		...
		with tf.device("/cpu:0"):
			...
			queue = tf.FIFOQueue(8, [tf.float32, tf.int32, tf.int32, tf.float32], name="input_queue")
			...
			self.inputs, self.input_lengths, self.split_infos, self.speaker_embeddings = queue.dequeue()
	...
	def getitem(self, split='train'):
		return np.randn([90, self._hparams.img_size, self._hparams.img_size, 3]), None
	...
	def _get_next_example(self):
		...
		return input_data, None, embed_target, None
	...
	def _prepare_batch(self, batches, outputs_per_step):
		...
		for i in range(self._hparams.tacotron_num_gpus):
			...
			split_infos.append([input_max_len, ])
		...
		return inputs, input_lengths, split_infos, embed_targets
