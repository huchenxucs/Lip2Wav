...
also support mydict['val'] get and set
"""
...
def process_audio_file(wavpath, args, gpu_id):
	fulldir = path.dirname(wavpath)
	specpath = path.join(fulldir, 'mels.npz')
...
def main(args):
	...
	wavpaths = glob(path.join(args.speaker_root, 'preprocessed/*/*/audio.wav'))

	jobs = [(vfile, args, i%args.ngpu) for i, vfile in enumerate(wavpaths)]
